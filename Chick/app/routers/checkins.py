...
from app.services.chicken_status import calc_chicken_status, chicken_exp_multiplier
...
MAX_ACCUM_MINUTES = 60         # 每次 checkin 最多累積 60 分鐘
...
def checkin_start(payload: CheckinStartIn, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
     # ✅ 新增：lat/lng 必填檢查（避免 None 直接炸）
    if payload.lat is None or payload.lng is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="lat/lng is required"
        )
...
def checkin_end(payload: CheckinEndIn, user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    ...
    rewarded_today = (
        ...
        .filter(
            ...
            Checkin.coins_awarded > 0,  # ✅ 只算有真的領到幣的
        )
        .count()
    )

    if rewarded_today >= MAX_DAILY_AWARDED_CHECKINS:
        # 今天已經領滿 2 次了：這次仍算成功打卡，但不再給幣
        row.status = CheckinStatus.verified
        row.reason = "DAILY_LIMIT_REACHED"
        row.coins_awarded = 0  # ✅ 明確寫 0，避免舊值殘留
        db.commit()
    ...
    # ✅ 如果 DB 原本就有 coins_awarded（例如重試時），不要被意外洗掉
    row.coins_awarded = max(int(row.coins_awarded or 0), int(awarded or 0))
    db.commit()
    ...
    if awarded > 0:
        ...
        if user:
            ...
            '''
            status = calc_chicken_status(db, user_id)
            multiplier = chicken_exp_multiplier(status)
